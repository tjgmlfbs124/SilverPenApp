...
public class BasicFieldNumberView extends AppCompatEditText implements FieldView {
            ...
            if (mNumberField != null) {
                try {
                    ...
                    if (text.length() > 0) {
                        ...
                        String textWithoutGrouping = text.toString().replace(mLocalizedGroupingSeparator, "");
                        try {
                            double newValue = mLocalizedNumberParser.parse(textWithoutGrouping).doubleValue();
    ...
    protected void onFinishInflate() {
        ...
        Log.i("HTS", "[BasicFieldNumberView]onFinishInflate");
...
//        setLayoutParams(lparams);
        ...
        mLocalizedGroupingSeparator = Character.toString(mLocalizedDecimalSymbols.getGroupingSeparator());
        mLocalizedNumberParser = new DecimalFormat("#.#", mLocalizedDecimalSymbols);
    ...
    protected void updateInputMethod() {
        Log.i("HTS", "[BasicFieldNumberView]updateInputMethod()");
...
//            setImeOptions(imeOptions);
            ...
            // EXTRACT_UI를 사용하지 않음! ( 키보드만 나타냄 ) @HTS
