...
public class BlocklyController {
    ...
    public BlockViewFactory getmViewFactory()
    {
        return mViewFactory;
    }
     ...
     * Code 갱신에 관련된 애들
     * mCodeEventInterface
     * setCodeEventInterface
     * AllField_BlockEvent
     */
    private CodeEventInterface mCodeEventInterface;
     ...
     * create, move 등에 관한 것.
     */
    public void setCodeEventInterface(CodeEventInterface listener)
    {
        mCodeEventInterface = listener;
    }

    /** @HTS
     * Field를 상속받은 모든 Block에 관해서는 이 함수가 호출되도록 구조를 구성함.
     *
     * ValueChange등 값 변경에 관한 것.
     */
    public void AllField_BlockEvent()
    {
//        Log.i("HTS", "BlocklyController~~~~ Code generate call!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        if(mCodeEventInterface != null)
            mCodeEventInterface.OnCodeChange();
    }
    ...
    public void addCallback(EventsCallback callback) {
        Log.i("HTS", "[" + TAG + "] addCallback(callback => " + callback);
...
//        Log.i("ABCDEF", "groupAndFireEvents!!!!!!!!!!!!!");
    ...
    public void addPendingEvent(BlocklyEvent event) {
        Log.i("HTS", "[" + TAG + "] addPendingEvent (event)=> " + event);
        // onRunCode가 들어갈 자리. @HTS
    ...
    @NonNull
    public void rebuildBlockView_Executing(Block b, boolean isEXE)
    {
        BlockView view = mHelper.getView(b);

        mViewFactory.rebuildBlockView(view, isEXE);
    }

    // 폐기.
    public void change_test(Block b)
    {
        if (mWorkspaceView != null) {
//            List<Block> rootBlocks = mWorkspace.getRootBlocks();
            ...
            ConnectionManager connManager = mWorkspace.getConnectionManager();
//            for (int i = 0; i < rootBlocks.size(); i++) {
                BlockGroup bg = mViewFactory.buildBlockGroupTree(b, connManager, mTouchHandler);
                mWorkspaceView.addView(bg);
//            }
        }
    }
     ...
     */
    public void bumpNeighborsRecursively(Block currentBlock, BlockGroup rootBlockGroup) {
    ...
    private boolean hasCallback(@BlocklyEvent.EventType int typeQueryBitMask) {
        Log.i("HTS", "[hasCallback] EventType => " + typeQueryBitMask);
    ...
    public static class Builder {
         ...
         * @return this
         */
        public Builder setToolboxUi(BlockListUI toolbox,CategorySelectorUI categoryUi) {
            mToolbox = toolbox;
