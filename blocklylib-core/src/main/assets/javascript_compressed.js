...
Blockly.JavaScript.init = function (a) {
	...
	if (a.length) {
		for (var c = 0; c < a.length; c++)
			b[c] = Blockly.JavaScript.variableDB_.getName(a[c].name, Blockly.Variables.NAME_TYPE);
...
Blockly.JavaScript.scrub_ = function (a, b) {
	...
	if (!a.outputConnection || !a.outputConnection.targetConnection) {
		...
		for (var e = 0; e < a.inputList.length; e++)
			a.inputList[e].type == Blockly.INPUT_VALUE && (d = a.inputList[e].connection.targetBlock()) && (d = Blockly.JavaScript.allNestedComments(d)) && (c +=
				Blockly.JavaScript.prefixLines(d, "// "))
	}
...
Blockly.JavaScript.controls_for = function (a) {
	...
	if (Blockly.isNumber(c) && Blockly.isNumber(d) &&
		...
		a = "", g = c, c.match(/^\w+$/) || Blockly.isNumber(c) || (g = Blockly.JavaScript.variableDB_.getDistinctName(b + "_start", Blockly.Variables.NAME_TYPE), a += "var " + g + " = " + c + ";\n"), c = d, d.match(/^\w+$/) || Blockly.isNumber(d) || (c = Blockly.JavaScript.variableDB_.getDistinctName(b + "_end", Blockly.Variables.NAME_TYPE),
...
Blockly.JavaScript.procedures_defreturn = function (a) {
	...
	Blockly.JavaScript.INFINITE_LOOP_TRAP && (c = Blockly.JavaScript.INFINITE_LOOP_TRAP.replace(/%1/g, "'" + a.id + "'") +
			c);
	(d = Blockly.JavaScript.valueToCode(a, "RETURN", Blockly.JavaScript.ORDER_NONE) || "") && (d = "  return " + d + ";\n");
...
Blockly.JavaScript.procedures_callreturn = function (a) {
	for (var b = Blockly.JavaScript.variableDB_.getName(a.getFieldValue("NAME"), Blockly.Procedures.NAME_TYPE), c = [], d = 0; d < a.arguments_.length; d++)
...
Blockly.JavaScript.text_charAt = function (a) {
	...
	case "FROM_END":
		return a =
			Blockly.JavaScript.getAdjusted(a, "AT", 1, !0),
...
//----------------------------------------------- WEEEMAKE -----------------------------------------------//
/**
 * @SEO WEEEMAKE 블록의 기능을 명세한다.
 * - base_delay : 기다리기
 * - weeemake_block : 모터제어 블록
 * - weeemake_buzzer : 부저제어 블록
...
Blockly.JavaScript.base_delay = function (a) {
	var delay_time = (Blockly.JavaScript.valueToCode(this, "DELAY_TIME", Blockly.JavaScript.ORDER_ATOMIC) || "0");
    console.log("[seo] delay_time : " + intToHex(delay_time));
	// return "BT_ARR.push('DELAY_" + delay_time + "');\n"

	var string = "serialArr.push('ff04" + intToHex(delay_time) + "23');";

	switch(BlocklyJavascriptInterface.getConnectRobot()){
        case "weeemake" :
            string = "definitionArray.push({x:0, y:0, protocol:'ff01" + intToHex(delay_time) + "23, blockname : 'base_delay'});\n";
            break;
        case "eggBean" :
            string = "definitionArray.push({x:0, y:0, protocol:'ff01" + intToHex(delay_time) + "23, blockname : 'base_delay'});\n";
            break;
        case "camRobot" :
            string = "definitionArray.push({x:0, y:0, protocol:'ff01" + intToHex(delay_time) + "0023, blockname : 'base_delay'});\n";
            break;
    }
	return string;
};

Blockly.JavaScript.weeemake_motor = function (a) {
	// console.log("[base_delay]a = ", a);
    var dir = this.getFieldValue("DIRECTION");
    var speed = this.getFieldValue("SPEED");
//    console.log("[seo] speed : " + speed);
//    console.log("[seo] dir: " + dir);

    var string;
	switch(BlocklyJavascriptInterface.getConnectRobot()){
        case "weeemake" :
            string = "definitionArray.push({x:0, y:0, protocol:'ff04" + dir + speed + "23, blockname : 'weeemake_motor'});\n";
            break;
        case "eggBean" :
            string = "definitionArray.push({x:0, y:0, protocol:'ff04" + dir + speed + "23, blockname : 'weeemake_motor'});\n";
            break;
        case "camRobot" :
            string = "definitionArray.push({x:0, y:0, protocol:'ff01" + dir + speed + "23, blockname : 'weeemake_motor'});\n";
            break;
	}
	return string;
};
Blockly.JavaScript.weeemake_buzzer = function (a) {
    var frequency = this.getFieldValue("FREQUENCY");
    var action = this.getFieldValue("ACTION");

//    console.log("[seo] frequency : " + frequency);
//    console.log("[seo] action: " + action);
	var string;
    switch(BlocklyJavascriptInterface.getConnectRobot()){
        case "weeemake" :
            if(action == "01") string = "definitionArray.push({x:0, y:0, protocol:'ff0504" + frequency +"0023, blockname : 'weeemake_buzzer'});\n"
            else string = "definitionArray.push({x:0, y:0, protocol:'ff0500000023, blockname : 'weeemake_buzzer'});\n";
            break;
        case "eggBean" :
            if(action == "01") string = "definitionArray.push({x:0, y:0, protocol:'ff0504" + frequency +"0023, blockname : 'weeemake_buzzer'});\n"
            else string = "definitionArray.push({x:0, y:0, protocol:'ff0500000023, blockname : 'weeemake_buzzer'});\n";
            break;
        case "camRobot" :
            // TODO action이 "00" 일때 프로토콜 전송할것
            if(action == "01") string = "definitionArray.push({x:0, y:0, protocol:'ff0502" + frequency +"23, blockname : 'weeemake_buzzer'});\n";
            else string = "definitionArray.push({x:0, y:0, protocol:'ff05000023, blockname : 'weeemake_buzzer'});\n";
            break;
    }
	return string;
};

Blockly.JavaScript.weeemake_boardRGB = function (a) {
    var string ;
    switch(BlocklyJavascriptInterface.getConnectRobot()){
        case "weeemake" :
            string = "definitionArray.push({x:0, y:0, protocol:'ff0200" + color +"23', blockname : 'face_bored_02'});\n"
            break;
        case "eggBean" :
            string = "definitionArray.push({x:0, y:0, protocol:'ff0200" + color +"23', blockname : 'face_bored_02'});\n"
            break;
        case "camRobot" :
            string = "definitionArray.push({x:0, y:0, protocol:'ff02" + color +"23', blockname : 'face_bored_02'});\n"
            break;
    }
//	console.log("seo string : " + string);
	return string;
};

Blockly.JavaScript.face_bored_01 = function (a) {
     var string ;
     switch(BlocklyJavascriptInterface.getConnectRobot()){
         case "weeemake" :
...
Blockly.JavaScript.face_bored_02 = function (a) {
     var string ;
...
Blockly.JavaScript.face_bored_03 = function (a) {
     var string ;
     switch(BlocklyJavascriptInterface.getConnectRobot()){
         case "weeemake" :
...
Blockly.JavaScript.face_bored_04 = function (a) {
     var string ;
     switch(BlocklyJavascriptInterface.getConnectRobot()){
         case "weeemake" :
...
Blockly.JavaScript.face_bored_05 = function (a) {
     var string ;
     switch(BlocklyJavascriptInterface.getConnectRobot()){
         case "weeemake" :
...
Blockly.JavaScript.face_bored_06 = function (a) {
     var string ;
     switch(BlocklyJavascriptInterface.getConnectRobot()){
         case "weeemake" :
...
Blockly.JavaScript.face_bored_07 = function (a) {
     var string ;
     switch(BlocklyJavascriptInterface.getConnectRobot()){
         case "weeemake" :
...
Blockly.JavaScript.face_bored_08 = function (a) {
     var string ;
     switch(BlocklyJavascriptInterface.getConnectRobot()){
         case "weeemake" :
...
Blockly.JavaScript.face_bored_09 = function (a) {
     var string ;
     switch(BlocklyJavascriptInterface.getConnectRobot()){
         case "weeemake" :
             string = "definitionArray.push({x:0, y:0, protocol:'ff0708000023', blockname : 'face_bored_09'});\n"
             break;
         case "eggBean" :
             string = "definitionArray.push({x:0, y:0, protocol:'ff0708000023', blockname : 'face_bored_09'});\n"
             break;
...
Blockly.JavaScript.mission_go = function (a) {
     var string = "definitionArray.push({x:0, y:1, protocol:'ff04005f0123', blockname : 'mission_go'});\n"
    return string;
};

Blockly.JavaScript.mission_left = function (a) {
     var string = "definitionArray.push({x:-1, y:0, protocol:'ff04025f0123', blockname : 'mission_left'});\n"
	return string;
};

Blockly.JavaScript.mission_right = function (a) { // BLOCK CODE - # 1002
     var string = "definitionArray.push({x:1, y:0, protocol:'ff04035f0123', blockname : 'mission_right'});\n"
    return string;
};

Blockly.JavaScript.mission_back = function (a) { // BLOCK CODE - # 1003
     var string = "definitionArray.push({x:0, y:-1, protocol:'ff04015f0123', blockname : 'mission_back'});\n"
    return string;
};
