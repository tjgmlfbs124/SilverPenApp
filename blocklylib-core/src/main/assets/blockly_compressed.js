...
goog.debug.enhanceError = function (a, b) {
	if (a instanceof Error)
		...
		c = Error(a), Error.captureStackTrace && Error.captureStackTrace(c, goog.debug.enhanceError);
...
goog.events.BrowserEvent.prototype.preventDefault = function () {
	...
	if (a.preventDefault)
		a.preventDefault();
	else if (a.returnValue = !1, goog.events.BrowserFeature.SET_KEY_CODE_TO_PREVENT_DEFAULT)
		try {
...
goog.html.TrustedResourceUrl.formatWithParams = function (a, b, c) {
	...
	b = /\?/.test(a) ? "&" : "?";
	for (var d in c)
		for (var e = goog.isArray(c[d]) ? c[d] : [c[d]], f = 0; f < e.length; f++)
			null != e[f] && (a += b + encodeURIComponent(d) + "=" + encodeURIComponent(String(e[f])), b = "&");
...
goog.style.installSafeStyleSheet = function (a, b) {
	...
	if (goog.userAgent.IE && d.createStyleSheet) {
		...
		d = c.getElementsByTagNameAndClass("HEAD")[0], d || (e = c.getElementsByTagNameAndClass("BODY")[0], d = c.createDom("HEAD"), e.parentNode.insertBefore(d, e)), e = c.createDom("STYLE"), goog.style.setSafeStyleSheet(e, a), c.appendChild(d, e);
	return e
};
...
goog.style.setBorderBoxSize = function (a, b) {
	...
	if (!goog.userAgent.IE || goog.userAgent.isVersionOrHigher("10") || d && goog.userAgent.isVersionOrHigher("8"))
		...
		c.pixelWidth = b.width, c.pixelHeight =
			b.height
};
...
goog.events.EventHandler.prototype.unlisten = function (a, b, c, d, e) {
	...
	else if (d = goog.isObject(d) ? !!d.capture : !!d, a = goog.events.getListener(a, b, c || this.handleEvent, d, e || this.handler_ || this))
...
Blockly.VariableMap.prototype.getVariable = function (a) {
	for (var b = Object.keys(this.variableMap_), c = 0; c < b.length; c++)
		for (var d = b[c], e = 0, f; f = this.variableMap_[d][e]; e++)
			if (Blockly.Names.equals(f.name, a))
				return f;
	return null
};
...
Blockly.VariableMap.prototype.getAllVariables = function () {
	for (var a = [], b = Object.keys(this.variableMap_), c = 0; c < b.length; c++)
		a = a.concat(this.variableMap_[b[c]]);
	return a
};
...
Blockly.Workspace.prototype.addTopBlock = function (a) {
	...
	if (this.isFlyout) {
		...
		for (var b = 0, c; c = a[b]; b++)
			this.getVariable(c) || this.createVariable(c)
	}
};
...
Blockly.Workspace.prototype.updateVariableStore = function (a) {
	if (!this.isFlyout) {
		...
		for (d = 0; a = c[d]; d++)
			this.getVariable(a.name) || this.createVariable(a.name, a.type, a.id)
	}
};
Blockly.Workspace.prototype.renameVariableInternal_ = function (a, b) {
	var c = this.getVariable(b);
	if (a && c && a.type != c.type)
		throw Error('Variable "' + a.name + '" is type "' + a.type + '" and variable "' + b + '" is type "' + c.type + '". Both must be the same type.');
	if (c && c.name != b)
		var d = c.name;
	Blockly.Events.setGroup(!0);
	c = this.getAllBlocks();
	for (var e = 0; e < c.length; e++)
		c[e].renameVar(a.name, b), d && c[e].renameVar(d, b);
...
Blockly.Comment.prototype.setVisible = function (a) {
	if (a != this.isVisible())
		if (Blockly.Events.fire(new Blockly.Events.Ui(this.block_, "commentOpen", !a, a)), !this.block_.isEditable() && !this.textarea_ || goog.userAgent.IE)
...
Blockly.Connection.prototype.connect_ = function (a) {
	...
	if (b.isConnected()) {
		...
		b.setShadowDom(null);
		if (e.isShadow())
			f = Blockly.Xml.blockToDom(e), e.dispose(), e = null;
...
Blockly.Extensions.register = function (a, b) {
	if (!goog.isString(a) || goog.string.isEmptyOrWhitespace(a))
...
Blockly.Extensions.apply = function (name, block, isMutator) {	// name, block, isMutator

	// if(name == "procedures_defnoreturn_mutator") return;
	
	console.log("name = " + name);
	console.log("block = " + block);
	console.log("isMutator = " + isMutator);
	
	var extensionFn = Blockly.Extensions.ALL_[name];
	console.log("extensionFn = " + extensionFn);
	...
	else
	{
		...
		console.log("mutatorProperties = " + mutatorProperties);
	}
	extensionFn.apply(block);
	if (isMutator)
		Blockly.Extensions.checkBlockHasMutatorProperties_('Error after applying mutator "' + name + '": ', block);
	else if (!Blockly.Extensions.mutatorPropertiesMatch_(mutatorProperties, block))
	{
...
Blockly.Tooltip.onMouseMove_ = function (a) {
	if (Blockly.Tooltip.element_ && Blockly.Tooltip.element_.tooltip && !Blockly.WidgetDiv.isVisible() && !Blockly.Tooltip.blocked_)
		if (Blockly.Tooltip.visible) {
			...
			Blockly.Tooltip.poisonedElement_ != Blockly.Tooltip.element_ && (clearTimeout(Blockly.Tooltip.showPid_), Blockly.Tooltip.lastX_ = a.pageX, Blockly.Tooltip.lastY_ = a.pageY, Blockly.Tooltip.showPid_ =
...
Blockly.ConnectionDB.prototype.searchForClosest = function (a, b, c) {
	...
	for (var g = b, h, k = f - 1; 0 <= k && this.isInYRange_(k, a.y_, b); )
		h = this[k], a.isConnectionAllowed(h, g) && (c = h, g = h.distanceFrom(a)), k--;
	for (; f < this.length && this.isInYRange_(f, a.y_, b); )
		h = this[f], a.isConnectionAllowed(h, g) && (c = h, g = h.distanceFrom(a)), f++;
	a.x_ = e;
	a.y_ = d;
	return {
		connection: c,
...
Blockly.utils.tokenizeInterpolation_ = function (a, b) {
	...
	for (h = f.length = 0; h < c.length; ++h)
		"string" == typeof c[h] ? f.push(c[h]) : ((k = f.join("")) && d.push(k), f.length = 0, d.push(c[h]));
	(k = f.join("")) && d.push(k);
	f.length = 0;
	return d
};
...
Blockly.utils.wrap = function (a, b) {
	for (var c = a.split("\n"), d = 0; d < c.length; d++)
		c[d] = Blockly.utils.wrapLine_(c[d], b);
	return c.join("\n")
};
...
Blockly.Options = function (a) {
	...
	if (b)
		...
		c = Blockly.Options.parseToolboxTree(a.toolbox), d = !(!c || !c.getElementsByTagName("category").length), e = a.trashcan, void 0 === e && (e = d), f = a.collapse, void 0 === f && (f = d), g = a.comments, void 0 === g && (g = d), h = a.disable, void 0 === h && (h = d), k = a.sounds, void 0 === k && (k = !0);
	var n = !!a.rtl,
...
Blockly.WorkspaceDragSurfaceSvg.prototype.clearAndHide = function (a) {
	...
	if (!(b && c && Blockly.utils.hasClass(b, "blocklyBlockCanvas") && Blockly.utils.hasClass(c, "blocklyBubbleCanvas")))
...
Blockly.Xml.workspaceToDom = function (a, b) {
	...
	for (var d = a.getTopBlocks(!0), e = 0, f; f = d[e]; e++)
...
Blockly.Xml.blockToDom = function (a, b) {
	...
	if (d = a.getCommentText())
		d = goog.dom.createDom("comment", null, d), "object" == typeof a.comment && (d.setAttribute("pinned", a.comment.isVisible()), e = a.comment.getBubbleSize(), d.setAttribute("h", e.height), d.setAttribute("w", e.width)), c.appendChild(d);
	...
	if (d = a.getNextBlock())
		k = goog.dom.createDom("next", null, Blockly.Xml.blockToDom(d, b)), c.appendChild(k);
...
Blockly.Xml.cloneShadow_ = function (a) {
	for (var b = a = a.cloneNode(!0), c; b; )
		if (b.firstChild)
			...
			for (; b && !b.nextSibling; )
				c = b, b = b.parentNode, 3 == c.nodeType && "" == c.data.trim() && b.firstChild != c && goog.dom.removeNode(c);
	...
	try {
		for (var h = 0; h < e; h++) {
			...
			if ("block" == n || "shadow" == n && !Blockly.Events.recordUndo) {
				...
				goog.asserts.fail("Shadow block cannot be a top-level block."), g = !1;
			else if ("variables" == n) {
				if (g)
					Blockly.Xml.domToVariables(k, b);
				else
...
Blockly.Xml.appendDomToWorkspace = function (a, b) {
	...
	if (d && d.height) {
		...
		for (d = 0; d <
			c.length; d++)
			b.getBlockById(c[d]).moveBy(b.RTL ? n - f : f, e)
	}
	return c
};
...
Blockly.Xml.domToBlockHeadless_ = function (a, b) {
	...
	for (var f = null, g = 0, h; h = a.childNodes[g]; g++)
		if (3 != h.nodeType) {
			...
			n = null;
			f = 0;
			for (var p; p = h.childNodes[f]; f++)
				1 == p.nodeType && ("block" == p.nodeName.toLowerCase() ? k = p : "shadow" == p.nodeName.toLowerCase() && (n = p));
			!k && n && (k = n);
			...
			case "field":
				...
				if (k instanceof
					...
					if (null !== p && p !== m.type)
						throw Error("Serialized variable type with id '" + m.getId() + "' had type " + m.type + ", and does not match variable field that references it: " + Blockly.Xml.domToText(h) + ".");
				}
				if (!k) {
	...
	if ("shadow" == a.nodeName.toLowerCase()) {
		d = c.getChildren();
		for (g = 0; e = d[g]; g++)
			goog.asserts.assert(e.isShadow(), "Shadow block not allowed non-shadow child.");
...
Blockly.ZoomControls.prototype.createDom = function () {
	...
	Blockly.bindEventWithChecks_(e, "mousedown", null, function (b) {
		console.log("Blockly.ZoomControls.prototype.createDom ---- event --- mouse down");
		a.markFocused();
		a.zoomCenter(1);
		Blockly.Touch.clearTouchIdentifier();
		b.stopPropagation();
...
Blockly.WorkspaceSvg.prototype.getSvgXY = function (a) {
	...
	c = 0,
	d = 1;
	if (goog.dom.contains(this.getCanvas(), a) || goog.dom.contains(this.getBubbleCanvas(), a))
		d = this.scale;
	do {
		var e = Blockly.utils.getRelativeXY(a);
...
Blockly.WorkspaceSvg.prototype.highlightBlock = function (a, b) {
	...
	if (d = a ? this.getBlockById(a) : null)
		(c = void 0 === b || b) ? -1 == this.highlightedBlocks_.indexOf(d) && this.highlightedBlocks_.push(d) : goog.array.remove(this.highlightedBlocks_, d), d.setHighlighted(c)
};
	...
	if (!this.options.readOnly && !this.isFlyout) {
		...
		if (this.options.collapse) {
			for (var k = h = !1, n = 0; n < e.length; n++)
				for (var p = e[n]; p; )
					p.isCollapsed() ? h = !0 : k = !0, p = p.getNextBlock();
			var l = function (a) {
...
Blockly.Mutator.prototype.resizeBubble_ = function () {
	...
	if (Math.abs(this.workspaceWidth_ - c) > a || Math.abs(this.workspaceHeight_ - b) > a)
		this.workspaceWidth_ = c, this.workspaceHeight_ = b, this.bubble_.setBubbleSize(c + a, b + a), this.svgDialog_.setAttribute("width", this.workspaceWidth_),
	...
	if (a != this.isVisible())
		if (Blockly.Events.fire(new Blockly.Events.Ui(this.block_, "mutatorOpen", !a, a)), a) {
			...
			if (a = this.workspace_.options.languageTree)
				this.workspace_.flyout_.init(this.workspace_), this.workspace_.flyout_.show(a.childNodes);
...
Blockly.Warning.prototype.setVisible = function (a) {
	if (a != this.isVisible())
		if (Blockly.Events.fire(new Blockly.Events.Ui(this.block_, "warningOpen", !a, a)), a) {
			...
			this.bubble_.dispose(), this.body_ = this.bubble_ = null
};
...
Blockly.Block.prototype.setConnectionsHidden = function (a) {
	if (!a && this.isCollapsed()) {
		...
		for (var c = this.getConnections_(!0), d = 0; b = c[d]; d++)
			b.setHidden(a), b.isSuperior() && (b = b.targetBlock()) && b.setConnectionsHidden(a)
};
...
Blockly.Block.prototype.setColour = function (a) {
	...
	else if (goog.isString(a) && a.match(/^#[0-9a-fA-F]{6}$/))
		this.colour_ = a, this.hue_ = null;
	else
		throw "Invalid colour: " + a;
};
Blockly.Block.prototype.setOnChange = function (a) {
	if (a && !goog.isFunction(a))
	...
	if (this.collapsed_)
		...
		for (var e = 0, f; f = this.inputList[e]; e++) {
			for (var g = 0, h; h = f.fieldRow[g]; g++)
				h instanceof Blockly.FieldDropdown && !h.getValue() ? c.push(d) : c.push(h.getText());
...
Blockly.Block.prototype.jsonInit = function (a) {
	...
	for (b = 0; void 0 !== a["message" + b]; )
		this.interpolate_(a["message" + b], a["args" + b] || [], a["lastDummyAlign" + b]), b++;
...
Blockly.Block.prototype.mixin = function (a, b) {
	if (goog.isDef(b) && !goog.isBoolean(b))
...
Blockly.Block.prototype.interpolate_ = function (a, b, c) {
	...
	for (var g = 0; g < d.length; g++) {
		...
		if ("number" == typeof h) {
			if (0 >= h || h > b.length)
				throw Error('Block "' + this.type + '": Message index %' + h + " out of range.");
			if (e[h])
				throw Error('Block "' + this.type + '": Message index %' + h + " duplicated.");
			e[h] = !0;
			f++;
			a.push(b[h - 1])
	...
	for (g = 0; g < a.length; g++)
		if (e = a[g], "string" == typeof e)
			b.push([e, void 0]);
		else {
			d = f = null;
			do
				if (h = !1, "string" == typeof e)
...
Blockly.BlockSvg.prototype.translate = function (a, b) {
	console.log("----- [Blockly.BlockSvg.prototype.translate] -----");
	console.log("a = ", a);
	console.log("b = ", b);
...
Blockly.BlockSvg.prototype.getBoundingRectangle = function () {
	...
	if (this.RTL) {
		...
		d = new goog.math.Coordinate(a.x - b, a.y), a = new goog.math.Coordinate(a.x + c.width - b, a.y + c.height);
	return {
		topLeft: d,
		bottomRight: a
	}
...
Blockly.BlockSvg.prototype.setCollapsed = function (a) {
	if (this.collapsed_ != a) {
		...
		if (a) {
			...
			this.removeInput("_TEMP_COLLAPSED_INPUT"), this.setWarningText(null);
...
Blockly.BlockSvg.prototype.setDragging = function (a) {
	if (a) {
		...
		Blockly.draggingConnections_ = [], Blockly.utils.removeClass(this.svgGroup_, "blocklyDragging");
...
Blockly.BlockSvg.prototype.updateColour = function () {
	if (!this.disabled) {
		...
		if (this.isShadow())
			b = goog.color.lighten(b, .6), a = goog.color.rgbArrayToHex(b), this.svgPathLight_.style.display = "none", this.svgPathDark_.setAttribute("fill", a);
		else {
...
Blockly.BlockSvg.prototype.setWarningText = function (a, b) {
	...
	var c = b || "";
	if (c)
		this.setWarningText.pid_[c] && (clearTimeout(this.setWarningText.pid_[c]), delete this.setWarningText.pid_[c]);
	else
		for (var d in this.setWarningText.pid_)
			clearTimeout(this.setWarningText.pid_[d]), delete this.setWarningText.pid_[d];
...
Blockly.BlockSvg.prototype.renderCompute_ = function (a) {
	...
	for (var d = 0, e = 0, f = !1, g = !1, h = !1, k = void 0, n = this.getInputsInline() && !this.isCollapsed(), p = 0, l; l = b[p]; p++)
		if (l.isVisible()) {
			if (n && k && k != Blockly.NEXT_STATEMENT && l.type != Blockly.NEXT_STATEMENT)
				var m = c[c.length - 1];
			else
				k = l.type, m = [], m.type = n &&
...
Blockly.BlockSvg.prototype.renderDraw_ = function (a, b) {
	...
	if (this.outputConnection)
		...
		if (this.previousConnection) {
			...
			Blockly.BlockSvg.START_HAT && (this.startHat_ = this.squareTopLeftCorner_ = !0, this.height += Blockly.BlockSvg.START_HAT_HEIGHT, b.rightEdge = Math.max(b.rightEdge,
						100));
...
Blockly.Events.filter = function (a, b) {
	...
	b || c.reverse();
	for (f = 1; g = c[f]; f++)
		g.type == Blockly.Events.CHANGE && "mutation" == g.element && c.unshift(c.splice(f, 1)[0]);
	return c
};
...
Blockly.Events.Create.prototype.run = function (a) {
	...
	if (a)
		a = goog.dom.createDom("xml"), a.appendChild(this.xml), Blockly.Xml.domToWorkspace(a, b);
	else {
		a = 0;
...
Blockly.Events.Delete.prototype.run = function (a) {
	...
	if (a) {
		...
		a = goog.dom.createDom("xml"), a.appendChild(this.oldXml), Blockly.Xml.domToWorkspace(a, b)
};
	...
	if (a.type == Blockly.Events.MOVE || a.type == Blockly.Events.CREATE) {
		...
		if (a = b.getBlockById(a.blockId))
			if (a.getParent() && !a.getParent().disabled) {
				...
				do
					a.setDisabled(!0), a = a.getNextBlock();
				while (a)
			}
		Blockly.Events.enable()
	}
...
Blockly.FieldTextInput.prototype.maybeSaveEdit_ = function () {
	...
	b = a.value;
	if (this.sourceBlock_)
		if (b = this.callValidator(b), null === b)
			b = a.defaultValue;
		else if (this.onFinishEditing_)
			this.onFinishEditing_(b);
	this.setText(b);
	this.sourceBlock_.rendered && this.sourceBlock_.render()
};
...
Blockly.Variables.flyoutCategoryBlocks = function (a) {
	...
	if (0 < a.length) {
		...
		for (c = 0; d = a[c]; c++)
			Blockly.Blocks.variables_get && (d = '<xml><block type="variables_get" gap="8">' + Blockly.Variables.generateVariableFieldXml_(d) + "</block></xml>", d = Blockly.Xml.textToDom(d).firstChild, b.push(d))
	}
	return b
};
...
Blockly.FieldVariable.prototype.setValue = function (a) {
	...
	if (this.sourceBlock_) {
		...
		if (d)
			c = d.name;
		else if (d = this.sourceBlock_.workspace.getVariable(a))
			b = d.getId();
...
Blockly.FieldVariable.prototype.getVariableTypes_ = function () {
	...
	if (null === a && this.sourceBlock_)
		return this.sourceBlock_.workspace.getVariableTypes();
	a = a || [""];
	if (0 == a.length)
		throw a = this.getText(), Error("'variableTypes' of field variable " + a + " was an empty list");
	return a
};
...
Blockly.Generator.prototype.workspaceToCode = function (a) {
	...
	// console.log("Blockly.Generator.prototype.workspaceToCode----- b = " + b);
	...
	// getTopBlocks의 parameter로 true로 넘기게 되면 y축 기준으로만 정렬한다.
	// false로 넘겨서 정렬을 하지 않도록 하고, workspaceToCode가 실행되기 전에 block들을
	// X ( 우선순위 1)
	// Y ( 우선순위 2)
	// 를 기준으로 정렬하여 넘긴다. @HTS
	a = a.getTopBlocks(false);
...
function rgb555(color){
	// split a hex color into red green and blue channels
	// e.g. '#FFFFFF' -> ['FF', 'FF', 'FF']
	var rgb = color.match(/[^#].{1}/g);
	// final hex values will be added to this array
	var hex = ['#'];

	// loop though each channel
	for (var i = 0; i < rgb.length; i++) {
	// convert the hex value to an integer, and scale it to a 5-bit value (between 0 and 31)
	// e.g. 'FF' -> 31 ,or '00' -> 0
	var c = (~~('0x' + rgb[i]) >> 3) & 0x1F;
	// scale 5-bit value back to 8-bit 
	// e.g. 31 -> 255, 0 -> 0
	c = (c << 3 | (c >> 2))
	// convert this value back to a hex string and then add it to the hex array
	hex.push(c.toString(16))
	}
	// join the array together into one string and return
	return hex.join('');
	// yay
}
...
function rgb565(rgb)
{
  ...
  var ret  = (R & 0xF8) << 8;  // 5 bits
	  ret |= (G & 0xFC) << 3;  // 6 bits
	  ret |= (B & 0xF8) >> 3;  // 5 bits
       
  return ("0000" + ret.toString(16)).substr(-4);
}

// color To code ---- BY.HTS
Blockly.Generator.prototype.colorToCode = function (a, b, c) {
	...
	// console.log("[colorToCode] a = ", a);
	// console.log("[colorToCode] a.parentBlock_.type = ", a.parentBlock_.type);
	
	if(a.parentBlock_.type.indexOf("LCD") != -1)
	{
		// console.log("LCD!!!!!");
		// b를 5bit rgb로 변환하자.
		// console.log("hexToRgb(b).r = ", hexToRgb(b).r);
		// console.log("hexToRgb(b).g = ", hexToRgb(b).g);
		// console.log("hexToRgb(b).b = ", hexToRgb(b).b);
		
		var hex = b[0].substring(2, 8);
		
		hex = "0x" + hex;
		
		// b = "0x" + rgb565(hex).toString(16);
		// url에 0x까지 묻어가게 되면 porotocol parameter가 꼬인다.
		b = rgb565(hex);
	}
	// else
	// {
...
Blockly.Names.prototype.getDistinctName = function (a, b) {
	for (var c = this.safeName_(a), d = ""; this.dbReverse_[c + d] || c + d in this.reservedDict_; )
		d = d ? d + 1 : 2;
	c += d;
	this.dbReverse_[c] = !0;
...
Blockly.FlyoutButton.prototype.createDom = function () {
	...
	if (!this.isLabel_)
		var b = Blockly.utils.createSvgElement("rect", {
...
Blockly.Flyout.prototype.show = function (a) {
	...
	for (var b = [], c = [], d = this.permanentlyDisabled_.length =
		...
		if (e.tagName) {
			...
			if ("BLOCK" == f)
				f = Blockly.Xml.domToBlock(e, this.workspace_), f.disabled && this.permanentlyDisabled_.push(f), b.push({
					type: "block",
					block: f
				}), e = parseInt(e.getAttribute("gap"), 10), c.push(isNaN(e) ? g : e);
			else if ("SEP" == e.tagName.toUpperCase())
				e = parseInt(e.getAttribute("gap"), 10), !isNaN(e) && 0 < c.length ? c[c.length - 1] = e : c.push(g);
			else if ("BUTTON" == f || "LABEL" == f)
				e = new Blockly.FlyoutButton(this.workspace_, this.targetWorkspace_,
						e, "LABEL" == f), b.push({
					type: "button",
					button: e
				}), c.push(g)
...
Blockly.Flyout.prototype.clearOldBlocks_ = function () {
	...
	for (b = 0; b < this.backgroundButtons_.length; b++)
		(a = this.backgroundButtons_[b]) && goog.dom.removeNode(a);
	for (b = this.backgroundButtons_.length = 0; a = this.buttons_[b]; b++)
		a.dispose();
...
Blockly.HorizontalFlyout.prototype.getClientRect = function () {
	...
	a = a.height;
	if (this.toolboxPosition_ == Blockly.TOOLBOX_AT_TOP)
		return new goog.math.Rect(-1E9, b - 1E9, 2E9, 1E9 + a);
	if (this.toolboxPosition_ == Blockly.TOOLBOX_AT_BOTTOM)
		return new goog.math.Rect(-1E9, b, 2E9, 1E9 + a)
};
...
Blockly.VerticalFlyout.prototype.layout_ = function (a, b) {
	...
	for (var c = this.MARGIN, d = this.RTL ? c : c + Blockly.BlockSvg.TAB_WIDTH, e = 0, f; f = a[e]; e++)
		if ("block" == f.type) {
			...
			"button" == f.type && (this.initFlyoutButton_(f.button, d, c), c +=
				f.button.height + b[e])
};
...
// Blockly.Css.CONTENT = [
	// ".blocklySvg {", "background-color: #fff;", "outline: none;", "overflow: hidden;", "position: absolute;", "display: block;", "}", ".blocklyWidgetDiv {", "display: none;", "position: absolute;", "z-index: 99999;", "}", ".injectionDiv {", "height: 100%;", "position: relative;", "overflow: hidden;", "touch-action: none", "}", ".blocklyNonSelectable {", "user-select: none;", "-moz-user-select: none;", "-webkit-user-select: none;", "-ms-user-select: none;", "}", ".blocklyWsDragSurface {", "display: none;",
...
"}",
".blocklyDraggable {",
	'cursor: url("<<<PATH>>>/handopen.cur"), auto;', "cursor: grab;",
	"cursor: -webkit-grab;",
	"cursor: -moz-grab;",
"}",
".blocklyDragging {",
...
Blockly.WidgetDiv.calculateX_ = function (a, b, c, d) {
	if (d)
		return b = Math.max(b.right - c.width, a.left), Math.min(b, a.right - c.width);
...
Blockly.bindEventWithChecks_ = function (a, b, c, d, e, f) {
	...
	h = function (a) {
		...
		for (var f = 0, h; h = a[f]; f++)
			if (!b || Blockly.Touch.shouldHandleEvent(h))
				Blockly.Touch.setClientFromTouch(h), c ? d.call(c, h) : d(h), g = !0
	};
	...
	if (b in Blockly.Touch.TOUCH_MAP)
		...
		}, p = 0, l; l = Blockly.Touch.TOUCH_MAP[b][p]; p++)
			a.addEventListener(l, n, !1), k.push([a, l, n]);
	return k
};
Blockly.bindEvent_ = function (a, b, c, d) {
	...
	if (b in Blockly.Touch.TOUCH_MAP)
		for (var g = function (a) {
			...
			a.addEventListener(k, g, !1), f.push([a, k, g]);
	return f
};
