...
public class BlockView extends AbstractBlockView<InputView> {
    ...
    public void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < mInputViews.size(); i++) {
            ...
            inputView.layout(rowFrom, rowTop, rowFrom + inputViewWidth, rowTop + inputView.getMeasuredHeight());
        }
    ...
    protected void onBlockUpdated(@Block.UpdateState int updateMask) {
        if ((updateMask & UPDATES_THAT_MIGHT_MODIFY_CHILDREN_OR_SIZE) != 0) {
            mFactory.rebuildBlockView(this, false);
        }
    }
     ...
     */
    public void layoutPatchesAndConnectors() {
        mBlockPatches.clear();
        ...
        for (int i = 0; i < mInputCount; ++i) {
            ...
            fillRectBySize(xFrom + inputLayoutOrigin.x,
                            inputLayoutOrigin.y,
                            inputView.getFieldLayoutWidth(),
                            inputView.getRowHeight());
//                                mBlock.isShadow() ? inputView.getRowHeight() - 20 : inputView.getRowHeight());

            int inputType = inputView.getInput().getType();
            boolean isLastInput = (i + 1 == mInputCount);
            boolean nextIsStatement = !isLastInput && mInputViews.get(i + 1).getInput().getType() == Input.TYPE_STATEMENT;
            boolean isEndOfLine = !mBlock.getInputsInline() || isLastInput || nextIsStatement;
            ...
            switch (inputType) {
                ...
                case Input.TYPE_VALUE: {
                    if (mBlock.getInputsInline()) {
                        addInlineValueInputPatch(isShadow, i, inlineRowIdx, xFrom, inputView, inputLayoutOrigin);
                    } else {
                ...
                case Input.TYPE_STATEMENT: {
                    ...
                    if (mBlock.getInputsInline()) {
                        ++inlineRowIdx;
                        xToBottom = xFrom + mInlineRowWidth.get(inlineRowIdx) - mPatchManager.mBlockStartPadding;
                    }

                    // Place the connector patches.
                    addStatementInputPatches(isShadow, i, xFrom, xTo, xToBottom, inputView, inputLayoutOrigin);
        ...
        final NinePatchDrawable bottomStartDrawable = getColoredPatchDrawable(bottomStartResourceId);
        final NinePatchDrawable bottomStartBorderDrawable = mPatchManager.getPatchDrawable(bottomStartBorderResourceId);
...
//                /* bottom */ mBlock.isShadow() ? mBlockViewSize.y - 20 : mBlockViewSize.y);
    ...
    private NinePatchDrawable addTopLeftPatch(boolean isShadow, int xTo, int yTop) {
        ...
        if (mBlock.getPreviousConnection() != null) {
            ...
            topStartDrawable = getColoredPatchDrawable(isShadow ? R.drawable.top_start_previous_shadow : R.drawable.top_start_previous);
            topStartBorderDrawable = mPatchManager.getPatchDrawable(R.drawable.top_start_previous_border);
            mPreviousConnectorHighlightPatch = mPatchManager.getPatchDrawable(R.drawable.top_start_previous_connection);
        } else if (mBlock.getOutputConnection() != null) {
            mHelper.setPointMaybeFlip(mOutputConnectorOffset, mOutputConnectorMargin, yTop);
            topStartDrawable = getColoredPatchDrawable(isShadow ? R.drawable.top_start_output_shadow : R.drawable.top_start_output);
            topStartBorderDrawable = mPatchManager.getPatchDrawable(R.drawable.top_start_output_border);
            mOutputConnectorHighlightPatch = mPatchManager.getPatchDrawable(R.drawable.top_start_output_connection);
        } else if (mUseCap) {
            topStartDrawable = getColoredPatchDrawable(isShadow ? R.drawable.top_start_hat_shadow : R.drawable.top_start_hat);
            topStartBorderDrawable = mPatchManager.getPatchDrawable(R.drawable.top_start_hat_border);
        } else {
            topStartDrawable = getColoredPatchDrawable(isShadow ? R.drawable.top_start_default_shadow : R.drawable.top_start_default);
            topStartBorderDrawable = mPatchManager.getPatchDrawable(R.drawable.top_start_default_border);
        }
...
//                /* top */ mBlock.isShadow() ? 10 : 0,
     ...
     */
    private void addDummyBoundaryPatch(boolean isShadow, int xTo, InputView inputView, ViewPoint inputLayoutOrigin) {
        // For external dummy inputs, put a patch for the block boundary.
        final NinePatchDrawable inputDrawable = getColoredPatchDrawable(isShadow ? R.drawable.dummy_input_shadow : R.drawable.dummy_input);
        final NinePatchDrawable inputBorderDrawable = mPatchManager.getPatchDrawable(R.drawable.dummy_input_border);
...
//                /* bottom */ mBlock.isShadow() ? inputLayoutOrigin.y + inputView.getRowHeight() - 20 : inputLayoutOrigin.y + inputView.getRowHeight());
...
//                /* bottom */ mBlock.isShadow() ? connectorBottom - 20 : connectorBottom);
        inputDrawable.setBounds(tempRect);
        inputBorderDrawable.setBounds(tempRect);
        ...
        if (i > 0) {
            ...
            mHelper.setRtlAwareBounds(tempRect,
                    ...
                    /* LTR end */patchRight,
                    /* bottom */ connectorTop);
//                    /* bottom */ mBlock.isShadow() ? connectorTop - 20 : connectorTop);
    ...
    private void addInlineValueInputPatch(boolean isShadow, int i, int inlineRowIdx, int blockFromX,
        ...
        final NinePatchDrawable inputDrawable = getColoredPatchDrawable(isShadow ? R.drawable.value_input_inline_shadow : R.drawable.value_input_inline);
        final NinePatchDrawable connectionHighlightDrawable = mPatchManager.getPatchDrawable(R.drawable.value_input_inline_connection);
        mHelper.setRtlAwareBounds(tempRect,
                /* this width */  mBlockViewSize.x,
                /* LTR start */ cutoutX,
                /* top */ cutoutY,
...
//                /* bottom */ mBlock.isShadow() ? cutoutY + inputView.getTotalChildHeight() - 20 : cutoutX + inputView.getTotalChildHeight());
        inputDrawable.setBounds(tempRect);
        connectionHighlightDrawable.setBounds(tempRect);
        mBlockPatches.add(inputDrawable);
        mInputConnectionHighlightPatches.set(i, connectionHighlightDrawable);
    ...
    private void finishFillRect() {
        if (mNextFillRect != null) {
            ...
            mNextFillRect.bottom = Math.min(mNextFillRect.bottom, mBlockContentHeight - mPatchManager.mBlockBottomPadding);
